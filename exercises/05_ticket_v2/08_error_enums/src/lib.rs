...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        ...
        Err(TicketNewError::InvalidTitle(msg)) => {
            panic!("{msg}");
        }
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::InvalidTitle("Title cannot be longer than 50 characters".to_string()));
        }
        if description.is_empty() {
            return Err(TicketNewError::InvalidDescription("Description cannot be empty".to_string()));
        }
        if description.len() > 500 {
            return Err(TicketNewError::InvalidDescription("Description cannot be longer than 500 characters".to_string()));
        }
